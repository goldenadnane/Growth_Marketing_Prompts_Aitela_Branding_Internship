...
import { AllPlans, SelectedPromptsDataEdit, AllCategorys, AllSubCategorys } from '@/type';
...
import AddPlans from '@/components/AdminComp/AddPlans';
import EditPlan from '@/components/AdminComp/EditPlans';
import ShowPlans from '@/components/AdminComp/ShowPlans';
...
const Plans = () => {
    const [items, setItems] = useState<AllPlans[]>();
    const [AllInformationPrompts, setAllInformationPrompts] = useState<AllInformationPromptsProps>({
        FetchAllCategorys: [],
        FetchAllSubCategorys: [],
    });

    const [RelaunchDataPlan, setRelaunchDataPlan] = useState(false);
    const [selectedPlan, setSelectedPlan] = useState<AllPlans>();
    const [isLoading, setIsLoading] = useState(true);
    const [ToggleBtnPlans, setToggleBtnPlans] = useState(false);
    const [ToggleBtnEditPlans, setToggleBtnEditPlans] = useState(false);
    const [ToggleBtnShowPrompts, setToggleBtnShowPrompts] = useState(false);
    const [Success, setSuccess] = useState(false);
    const [search, setSearch] = useState('');
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<AllPlans[] | undefined>([]);
    const [records, setRecords] = useState<AllPlans[] | undefined>(initialRecords);
    const [selectedRecords, setSelectedRecords] = useState<any>([]);
    const ApiUrl = process.env.NEXT_PUBLIC_BASE_URL;
    const [page, setPage] = useState(1);
    const [AlertDelete, setAlertDelete] = useState(false);
    const [PopupDeleteValue, setPopupDeleteValue] = useState(false);
    const [PromptToDelete, setPromptToDelete] = useState<AllPlans | undefined>();
    ...
    const toggleOpenClose = () => {
        setToggleBtnPlans((prev) => !prev);
    };
    const toggleOpenCloseEdit = () => {
        setToggleBtnEditPlans((prev) => !prev);
    };
    ...
    const TogglePopUpDeletePrompt = (Plans: AllPlans) => {
        setPopupDeleteValue((prev) => !prev);
        setPromptToDelete(Plans);
    };
    const ToggleSuccess = () => {
    ...
    // Delete Prompt
    const DeletePlanshowAlert = async (type: string, Plan: AllPlans) => {
        const { id } = Plan;
        if (type === 'delete plan') {
            ...
            if (result.value && id) {
                try {
                    ...
                    setSearch('');
                    ToggleAlertDelete();
                    TogglePopUpDeletePrompt(Plan);
                    Swal.fire({ text: `The Plan has been deleted.`, icon: 'success', customClass: 'sweet-alerts' });
                } catch (error) {
                    console.error('Error deleting user:', error);
                    Swal.fire({ text: `The Plan has not been deleted.`, icon: 'warning', customClass: 'sweet-alerts' });
                }
            } else {
    ...
    useEffect(() => {
        if (ToggleBtnPlans || ToggleBtnEditPlans) {
            document.body.classList.add('overflow-hidden');
        } else {
            document.body.classList.remove('overflow-hidden');
        }
    }, [ToggleBtnPlans, ToggleBtnEditPlans]);
    ...
    // Selected the Plan
    const selectPlanEdit = (plan: AllPlans) => {
        const { id, name, price, pre_made_prompt, chat_per_day, saved_prompt, custom_instructions, is_available, payment_cycle, currency } = plan;
        const selectedUserData = { id, name, price, pre_made_prompt, chat_per_day, saved_prompt, custom_instructions, is_available, payment_cycle, currency };
        setSelectedPlan(selectedUserData);
    };
            ...
            <>
                {ToggleBtnPlans ? (
                    <AddPlans
                        ...
                        setRelaunchDataPlan={setRelaunchDataPlan}
                        items={items}
                    />
                ) : (
                    ''
                ...
                {ToggleBtnEditPlans ? (
                    <EditPlan items={items} setRelaunchDataPlan={setRelaunchDataPlan} toggleOpenCloseEdit={toggleOpenCloseEdit} selectedPlan={selectedPlan} setSelectedPlan={setSelectedPlan} />
                ) : (
                    ''
                )}
                {ToggleBtnShowPrompts ? (
                    <ShowPlans items={items} setRelaunchDataPlan={setRelaunchDataPlan} toggleOpenCloseShow={toggleOpenCloseShow} selectedPlan={selectedPlan} setSelectedPlan={setSelectedPlan} />
                ) : (
                    ''
                )}
                ...
                <div className="relative mb-6 mt-4 space-y-1 lg:mt-8 lg:space-y-4">
                    <h1 className="text-lg font-bold md:text-xl lg:text-2xl">Plans</h1>
                                    ...
                                    columns={[
                                        {
                                            ...
                                            render: ({ name }) => <div className="font-semibol text-center underline hover:no-underline">{name}</div>,
                                        },
                                        {
                                            accessor: 'Status',
                                            textAlignment: 'center',
                                            render: ({ is_available }) => (
                                                <div
                                                    className={`${
                                                        is_available === true ? 'bg-[#00ab563e]' : 'bg-[#ea443538]'
                                                    } m-auto  flex w-[95px] items-center justify-center rounded-3xl px-4 py-1 lg:py-2`}
                                                >
                                                    <span className={`${is_available === true ? 'text-[#00AB55]' : 'text-[#EA4335]'} font-bold`}>{is_available === true ? 'Active' : 'Desactive'}</span>
                                                </div>
                                            ),
                                        },
                                        {
                                            accessor: 'Price',
                                            textAlignment: 'center',
                                            render: ({ price }) => <div className="m-auto w-fit text-left font-semibold">{`$${price.toFixed(2)}`}</div>,
                                        },
                                        {
                                            accessor: 'Payment Cycle',
                                            textAlignment: 'center',
                                            render: ({ payment_cycle }) => <div className="m-auto w-fit  text-left font-semibold">{payment_cycle}</div>,
                                        },
                                        {
                                            accessor: 'action',
                                            title: 'Actions',
                                                    ...
                                                    {isDesktop ? (
                                                        <div className="mx-auto flex w-max items-center gap-4">
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    DeletePlanshowAlert('delete plan', plan);
                                                                }}
                                                            >
